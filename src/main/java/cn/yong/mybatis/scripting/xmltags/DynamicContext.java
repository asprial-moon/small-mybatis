...
public class DynamicContext {
    ...
    static {
        ...
        // 将传入的参数对象用以封装为ContextMap对象(继承了HashMap对象),
        // 然后Ognl运行时环境在动态计算sql时
        // 会按照ContextAccessor中描述的Map接口的方式来访问和读取ContextMap对象，获取计算过程中需要的参数
        // ContextMap对象内部可能封装了一个普通的POJO对象，也可以直接传递的Map对象，当然从外部是看不出来的，因为都是使用Map的接口来读取数据。
    }
